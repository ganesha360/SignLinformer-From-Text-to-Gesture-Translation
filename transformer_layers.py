...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, k: Tensor, v: Tensor, q: Tensor, mask: Tensor = None, padding_mask: Tensor = None):
        ...
        # reshape projected k and v 
        k_proj = k_proj.view(batch_size, -1, num_heads, self.head_size).transpose(1, 2)
        v_proj = v_proj.view(batch_size, -1, num_heads, self.head_size).transpose(1, 2)
        ...
        context = torch.matmul(attention, v_proj)
...
# Rest of the code remains exactly the same as the original implementation
# Only MultiHeadedAttention class is modified to use Linformer projection

# Copy-paste the rest of the original classes: 
